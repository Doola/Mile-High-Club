...
public class KdTree<T extends KdTree.XYZPoint> {
	 ...
	 */
	private static final <T extends KdTree.XYZPoint> KdNode getNode(
			KdTree<T> tree, T value) {
	...
	public Collection<T> nearestNeighbourSearch(int K, T value) {
		...
		TreeSet<KdNode> results = new TreeSet<KdNode>(new EuclideanComparator(
				value));
	...
	private static final <T extends KdTree.XYZPoint> void searchNode(T value,
			KdNode node, int K, TreeSet<KdNode> results, Set<KdNode> examined) {
		examined.add(node);
		...
		if (lesser != null && !examined.contains(lesser)) {
			...
			boolean lineIntersectsCube = ((valuePlusDistance <= nodePoint) ? true
					: false);
	...
	protected static class TreePrinter {
		...
		private static String getString(KdNode node, String prefix,
				boolean isTail) {
			StringBuilder builder = new StringBuilder();
